...
var includesIgnore$1 =  function(tpl) {
...
var utils$3 = {
	includesIgnore: includesIgnore$1
};
...
var tree = class Tree {
  ...
  setDependency(children, issuer, filepath) {
    for (const dependency of children) {
      ...
      if (path === issuer) {
        return dependency.add(new Dependency({ path: filepath,name: path__default['default'].relative(this.name,filepath) }));
      }
...
bytes_1.format = format_1$2;
bytes_1.parse = parse_1$4;
...
var Buffer$a = buffer__default['default'].Buffer;
...
function getDecoder (encoding) {
  ...
  try {
    return lib$1.getDecoder(encoding)
  } catch (e) {
    // error getting decoder
    if (!ICONV_ENCODING_MESSAGE_REGEXP.test(e.message)) throw e

    // the encoding was not found
    throw httpErrors$1(415, 'specified encoding unsupported', {
      encoding: encoding,
...
function readStream (stream, encoding, length, limit, callback) {
  ...
  if (limit !== null && length !== null && length > limit) {
    return done(httpErrors$1(413, 'request entity too large', {
      expected: length,
      length: length,
      limit: limit,
  ...
  if (stream._decoder || (state && (state.encoding || state.decoder))) {
    // developer error
    return done(httpErrors$1(500, 'stream encoding should not be set', {
  ...
  function onAborted () {
    ...
    done(httpErrors$1(400, 'request aborted', {
      code: 'ECONNABORTED',
      expected: length,
      length: length,
      received: received,
  ...
  function onData (chunk) {
    ...
    if (limit !== null && received > limit) {
      done(httpErrors$1(413, 'request entity too large', {
        limit: limit,
        received: received,
        type: 'entity.too.large'
      }));
  ...
  function onEnd (err) {
    ...
    if (length !== null && received !== length) {
      done(httpErrors$1(400, 'request size did not match content length', {
        expected: length,
        length: length,
        received: received,
...
function read (req, res, next, parse, debug, options) {
  ...
  if (opts.encoding === null && encoding !== null && !lib$1.encodingExists(encoding)) {
    return next(httpErrors$1(415, 'unsupported charset "' + encoding.toUpperCase() + '"', {
  ...
  rawBody(stream, opts, function (error, body) {
    if (error) {
      ...
      if (error.type === 'encoding.unsupported') {
        // echo back charset
        _error = httpErrors$1(415, 'unsupported charset "' + encoding.toUpperCase() + '"', {
          charset: encoding.toLowerCase(),
          type: 'charset.unsupported'
        });
      } else {
        // set status code on error
        _error = httpErrors$1(400, error);
      }

      // read off entire request
      stream.resume();
      onFinished_1(req, function onfinished () {
        next(httpErrors$1(400, _error));
      });
      return
    }
...
function contentstream (req, debug, inflate) {
  ...
  if (inflate === false && encoding !== 'identity') {
    throw httpErrors$1(415, 'content encoding unsupported', {
      encoding: encoding,
      type: 'encoding.unsupported'
    })
  }
  ...
  switch (encoding) {
    ...
    case 'identity':
      ...
      break
    default:
      throw httpErrors$1(415, 'unsupported content encoding "' + encoding + '"', {
        encoding: encoding,
        type: 'encoding.unsupported'
      })
  }
...
var debug$8 = src$1('body-parser:json');
...
function json (options) {
  ...
  return function jsonParser (req, res, next) {
    ...
    if (charset.substr(0, 4) !== 'utf-') {
      ...
      next(httpErrors$1(415, 'unsupported charset "' + charset.toUpperCase() + '"', {
        charset: charset,
        type: 'charset.unsupported'
      }));
      return
...
var debug$7 = src$1('body-parser:raw');
...
var debug$6 = src$1('body-parser:text');
...
var debug$5 = src$1('body-parser:urlencoded');
...
function urlencoded (options) {
  ...
  if (opts.extended === undefined) {
    deprecate$3('undefined extended: provide extended option');
  }
...
function extendedparser (options) {
  ...
  return function queryparse (body) {
    ...
    if (paramCount === undefined) {
      ...
      throw httpErrors$1(413, 'too many parameters', {
...
var debug$4 = src$1('finalhandler');
...
var defer = typeof setImmediate === 'function'
  ? setImmediate
  : function (fn) { process.nextTick(fn.bind.apply(fn, arguments)); };
var isFinished = onFinished_1.isFinished;
...
var debug$3 = src$1('express:router:layer');
...
var debug$2 = src$1('express:router:route');
...
var debug$1 = src$1('express:view');
...
var basename$1 = path$1.basename;
var extname$2 = path$1.extname;
...
SendStream.prototype.redirect = function redirect (path) {
  ...
  var loc = encodeurl(collapseLeadingSlashes$1(this.path + '/'));
...
var express$1 = createCommonjsModule(function (module, exports) {
...
var opener = function opener(args, options, callback) {
    ...
    if (platform === "linux" && os__default['default'].release().indexOf("Microsoft") !== -1) {
        platform = "win32";
    }
    ...
    return childProcess__default['default'].execFile(command, args, options, callback);
};
...
path__default['default'].resolve(__dirname, 'views');
path__default['default'].join(projectRoot, 'public');
...
class ModuleTreePlugin {
  ...
  apply(compiler) {
    ...
    if (typeof entry === 'string') {
      this.entry = { main: [path__default['default'].resolve(entry)] };
    } else if (Array.isArray(entry)) {
      this.entry = { main: entry.map(i=>path__default['default'].resolve(i))};
